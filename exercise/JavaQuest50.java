...
public class JavaQuest50 {
  ...
  public static int splitNum(int num) {
    ...
    for (int i = 0; i < String.valueOf(num).length(); i++) {
      if (i % 2 == 0) {
        even.add(String.valueOf(num).charAt(i));
      } else
        odd.add(String.valueOf(num).charAt(i));
    }
    ...
    for (char ch : even) {
      evv += String.valueOf(ch);
    }
    for (char ch : odd) {
      od2 += String.valueOf(ch);
    }
    int evnum = Integer.valueOf(evv);
    int odnum = Integer.valueOf(od2);


    // Integer ev2 = Integer.valueOf(String.valueOf(even));
